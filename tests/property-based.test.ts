...
describe('Property-based: Arithmetic', () => {
  ...
  describe('subtraction properties', () => {
    ...
    it('negation consistency: a - b = a + (-b) for non-negative b', () => {
      ...
      fc.assert(
        fc.property(safeInt, fc.integer({ min: 0, max: 1000000 }), (a, b) => {
...
describe('Property-based: Logical', () => {
  ...
  describe('De Morgan laws', () => {
    it('!(p && q) = !p || !q', () => {
      fc.assert(
        fc.property(fc.boolean(), fc.boolean(), (p, q) => {
...
describe('Property-based: Precedence', () => {
  ...
  it('AND binds tighter than OR', () => {
    fc.assert(
      fc.property(fc.boolean(), fc.boolean(), fc.boolean(), (p, q, r) => {
        const left = parser
          .parseExpression(`${String(p)} || ${String(q)} && ${String(r)}`)
          .getValue();
        const right = parser
          .parseExpression(`${String(p)} || (${String(q)} && ${String(r)})`)
          .getValue();
